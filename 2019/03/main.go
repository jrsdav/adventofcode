...
func main() {
	closest, furthest, err := adventDistance(input)
	if err != nil {
		log.Fatal(err)
	}
	fmt.Printf("Closest intersection coordinate: %v\nFurthest intersection coordinate: %v\n", closest, furthest)
}

func adventDistance(input string) (int, int, error) {
	parsed := strings.Split(input, "\n")

	grid := make(map[Coordinate]Position)
	...
	furthest := math.MaxFloat64
	for loc, hits := range grid {
	...
	return int(nearest), int(furthest), nil
}
